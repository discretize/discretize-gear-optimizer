...
pub fn start(
    ...
    let total_combinations = get_total_combinations(combinations);
...
pub fn start_with_heuristics(
    ...
    for (index, setting) in settings.iter().enumerate() {
        for _ in 0..BENCHMARK_ITERATIONS_PER_SETTING {
            let mut character = Character::new(setting.rankby);
            character.combination_id = index as u32;

            let gear = get_random_affix_combination(&setting.affixesArray, setting.slots as usize);

            // calculate stats for this combination
            let valid = test_character(&mut character, setting, &gear);
            if valid {
                // insert into result_characters if better than worst character
                result.insert(&character);
            }
    ...
    let weighted_combinations = result.get_weighted_combinations(&settings);

    let mut picked_settings: Vec<Settings> = Vec::new();
    // only pick those combinations that have at least a 10% likelyhood of being "good"
    for (index, setting) in settings.iter().enumerate() {
        if weighted_combinations[index] > 10 {
            picked_settings.push(setting.clone());
        }
    }

    return start(chunks, &picked_settings, workerglobal);
}
